...
# Variables
$UrlSimulation = "${BaseUrl}:$PortSimulation/"
$UrlWebApp = "${BaseUrl}:$PortWebapp/"
$DirSimulation = Join-Path -Path $PSScriptRoot -ChildPath "simulation"
$DirWebApp = Join-Path -Path $PSScriptRoot -ChildPath "webapp" | Join-Path -ChildPath "quick-demo"
...
foreach ($url in $UrlSimulation, $UrlWebApp)
{
	Start-Process $url
}
